...
- Post figure helpers (raw HTML in markdown): `<figure class="figure-float">` floats a portrait image right with text wrapping (full width on mobile); `<figure class="figure-single figure-zoom">` makes a dense image (poster, diagram) click-to-expand in a full-viewport lightbox with 1:1 zoom (built by `Post.astro`, styled in `global.css`).
